...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'InnovaTron.settings')  # Correct reference
# Initialize Django
django.setup()
...
MODEL_PATH = os.getenv("MODEL_PATH", "D:\Innova-Tron\openvino_env")  # Replace with your model's path
ie = Core()
try:
...
def translate_text(text, source_lang="en", target_lang="hi"):
    ...
    if not tokenizer or not translation_model:
        raise ValueError("Translation model not loaded.")
    # Ensure the target language is Hindi
    if target_lang != "hi":
